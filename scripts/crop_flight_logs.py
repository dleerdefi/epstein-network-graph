#!/usr/bin/env python3
"""
Crop flight log pages to remove excess white space and improve extraction quality.
This script applies consistent cropping to all pages in the flight logs directory.
"""
...
def process_directory(input_dir, output_dir, boundaries, test_pages=None):
    ...
    # Get all PNG files
    if test_pages:
        # Process only specified test pages (flight logs use page-XXX.png format)
        png_files = []
        for page_num in test_pages:
            page_file = input_path / f"page-{page_num:03d}.png"
            if page_file.exists():
                png_files.append(page_file)
            else:
                print(f"Warning: page-{page_num:03d}.png not found")
    else:
...
def main():
    parser = argparse.ArgumentParser(description='Crop flight log pages to improve extraction')
    parser.add_argument('--input', '-i',
                       default='data/source/flight_logs',
                       help='Input directory with original images')
    parser.add_argument('--output', '-o',
                       default='data/source/flight_logs_cropped',
                       help='Output directory for cropped images')
    parser.add_argument('--left', type=int, default=100,
                       help='Left boundary for crop (default: 100)')
    parser.add_argument('--top', type=int, default=500,
                       help='Top boundary for crop (default: 500) - removes top 20%')
    parser.add_argument('--right', type=int, default=3200,
                       help='Right boundary for crop (default: 3200)')
    parser.add_argument('--bottom', type=int, default=2450,
                       help='Bottom boundary for crop (default: 2450)')
